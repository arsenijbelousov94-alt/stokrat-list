...
async function handleTelegramWebhook(request, env) {
  ...
  if (text.startsWith('/status')) {
    ...
    if (idx === -1) {
      await sendTelegramMessage(env, chatId, `Ничего не найдено по «${query}».`, message.message_thread_id);
      return json({ ok: true });
    }
...
async function recordSeenUser(env, chatId, tgUser) {
  ...
  const entry = seen[username] || { userId: tgUser.id, chats: {} };
  entry.userId = tgUser.id;
