...
async function syncGroupStatus(env) {
  ...
  if (result.ok && result.messageId) {
    ...
    await kv.put('group_status_last_sent', new Date().toISOString());
  }
}
...
async function maybeRefreshGroupStatus(env) {
  ...
  const lastSent = await kv.get('group_status_last_sent');
  const ONE_HOUR = 60 * 60 * 1000;
  if (lastSent && Date.now() - new Date(lastSent).getTime() < ONE_HOUR) return;
