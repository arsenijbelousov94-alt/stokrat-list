...
async function handleTelegramWebhook(request, env) {
  ...
  // Нажатие на inline-кнопку (Принять/Отклонить заявку, и т.п.)
...
async function recordSeenUser(env, chatId, tgUser) {
  ...
  // Индекс по user_id — чтобы заметить смену юза у уже известного человека
  const byId = JSON.parse((await kv.get('seen_users_by_id')) || '{}');
  const prevEntry = byId[userId];
  if (prevEntry && prevEntry.username && newUsername && prevEntry.username !== newUsername) {
    const people = JSON.parse((await kv.get('people')) || '[]');
  ...
  byId[userId] = { username: newUsername, lastSeen: new Date().toISOString() };
