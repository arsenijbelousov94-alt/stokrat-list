...
async function sendRoleMenu(env, chatId) {
  const keyboard = {
    inline_keyboard: [
...
async function handleTelegramWebhook(request, env) {
  ...
  if (update.callback_query) {
    ...
    if (data.startsWith('role_pick:')) {
      ...
      if (match) {
        const requests = JSON.parse((await kv.get('role_requests')) || '[]');
        const alreadyRequested = requests.some((r) => r.personId === match.id && r.roleCode === roleCode);
        if (!alreadyRequested) {
          requests.push({
            id: crypto.randomUUID(),
            personId: match.id,
            ...
            roleCode,
            roleLabel,
            chatId: String(cqChatId),
            username: tgUser.username,
            requestedAt: new Date().toISOString()
          });
          await kv.put('role_requests', JSON.stringify(requests));
        }

        await answerCallbackQuery(env, cq.id, 'Заявка отправлена на рассмотрение');
        await sendTelegramMessage(env, cqChatId, `📨 Ваша заявка на роль «${roleLabel}» отправлена модераторам. С вами свяжутся.`);

        const bindings2 = JSON.parse((await kv.get('telegram_bindings')) || '{}');
        const notifyText = `🎭 Новая заявка на роль!\n\nНик: ${match.nick || '—'}\nЮз: ${match.uz || '—'}\nРоль: ${roleLabel}\n\nПроверьте вкладку «Заявки на роль» на сайте.`;
        for (const adminChatId of Object.values(bindings2)) {
          await sendTelegramMessage(env, adminChatId, notifyText);
        }
        await notifyPushSubscribers(env, '🎭 Заявка на роль', `${match.nick || username}: ${roleLabel}`, '/');
      } else {
  ...
  if (text.startsWith('/start')) {
    ...
    // Обычный пользователь — предлагаем заполнить анкету, и сразу даём кнопку «стать сотрудником»
    const staffKeyboard = {
      inline_keyboard: [[{ text: '🎭 Хотите стать сотрудником?', callback_data: 'show_role_menu' }]]
    };
    await sendTelegramMessage(env, chatId, APPLICATION_TEMPLATE, null, staffKeyboard);
    const flow = JSON.parse((await kv.get('app_flow_state')) || '{}');
...
async function handleApi(request, env) {
  ...
  if (body.action === 'list_role_requests') {
    const requests = JSON.parse((await kv.get('role_requests')) || '[]');
    return json(requests);
  }

  if (body.action === 'accept_role_request') {
    if (!canModerate(auth)) return json({ error: 'Недостаточно прав' }, 403);
    if (!body.id) return json({ error: 'id обязателен' }, 400);
    const requests = JSON.parse((await kv.get('role_requests')) || '[]');
    const entry = requests.find((r) => r.id === body.id);
    if (!entry) return json({ error: 'Заявка не найдена' }, 404);

    const remaining = requests.filter((r) => r.id !== body.id);
    await kv.put('role_requests', JSON.stringify(remaining));

    const people = JSON.parse((await kv.get('people')) || '[]');
    const person = people.find((p) => p.id === entry.personId);
    if (person) {
      const oldPost = person.post;
      person.post = entry.roleLabel;
      await kv.put('people', JSON.stringify(people));
      await appendLog(env, {
        actor: auth.login, action: 'update', personId: person.id, nick: person.nick,
        changes: [{ field: 'post', from: oldPost, to: entry.roleLabel }]
      });
    }

    await sendTelegramMessage(env, entry.chatId, `✅ Ваша заявка на роль «${entry.roleLabel}» принята! С вами свяжутся.`);
    return json({ ok: true, person });
  }

  if (body.action === 'reject_role_request') {
    if (!canModerate(auth)) return json({ error: 'Недостаточно прав' }, 403);
    if (!body.id) return json({ error: 'id обязателен' }, 400);
    const requests = JSON.parse((await kv.get('role_requests')) || '[]');
    const entry = requests.find((r) => r.id === body.id);
    ...
    await sendTelegramMessage(env, entry.chatId, `❌ Ваша заявка на роль «${entry.roleLabel}» отклонена.`);
    return json({ ok: true });
  }
